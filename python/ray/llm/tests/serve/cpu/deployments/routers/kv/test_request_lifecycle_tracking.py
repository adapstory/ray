...
class FakeSelectionService:
    ...
    hooks make, standing in for the real pyo3 service on CPU.

    ``create_reservation``/``prefill_complete``/``free_reservation`` are async
    as in the real binding; ``add_output_block`` is synchronous.
    """
...
class LifecycleActor(KVRouterActor):
    """In-process KVRouterActor with the event plane + Serve LongPoll stripped
    and a FakeSelectionService recording the reservation calls its lifecycle
    hooks make."""
...
async def test_lifecycle_books_selection_service_load(build_token_tracking_engine):
    ...
    assert calls[0] == ("create_reservation", "req-1", WORKER_ID, 12, None)
...
async def test_decode_block_crossings_drive_add_output_block():
    ...
    # Three crossings -> three add_output_block calls, after one reservation.
    assert (
...
async def test_expected_output_tokens_set_decay_fraction():
    """With an output-length estimate, each decode block decays by the remaining
...
async def test_in_order_reports(build_token_tracking_engine):
    ...
    assert names[0] == "create_reservation"
    assert names[1] == "prefill_complete"
    assert names[-1] == "free_reservation"


@pytest.mark.asyncio
@pytest.mark.parametrize("early_drop", [False, True])
async def test_freed_exactly_once(early_drop, build_token_tracking_engine):
    """free_reservation fires exactly once on normal end and on early close."""
    actor = RecordingKVRouterActor.remote(block_size=16)
    ...
    calls = ray.get(actor.get_dynamo_calls.remote())
    assert op_names(calls).count("free_reservation") == 1
...
async def test_engine_error_still_frees(build_token_tracking_engine):
    ...
    assert op_names(ray.get(actor.get_dynamo_calls.remote()))[-1] == "free_reservation"
...
async def test_zero_output_request_added_then_freed(build_token_tracking_engine):
    ...
    assert op_names(ray.get(actor.get_dynamo_calls.remote())) == [
        "create_reservation",
        "free_reservation",
    ]
...
async def test_hooks_for_unknown_request_book_nothing():
    """Hooks for a never-admitted request id touch neither state nor the service."""
    actor = LifecycleActor(block_size=16)
...
async def test_active_load_views():
    ...
    assert ("free_reservation", "a") in actor._svc.calls
